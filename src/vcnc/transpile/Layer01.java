...
Layer00 handles subroutine calls and returns. It removes M98 and M99 from the
  input stream, along with a bunch of things that I don't handle or that 
  don't mean anything in a virtual context, like coolant on/off.
  BUG: Does it remove that other stuff?

This layer converts everything to inches or mm, whichever is the default
metric for this machine. It also needs to keep track of whether we are in
polar coordinate mode since polar coordinates may include an angle, and
angles are not affected by the inch/mm choice. Moreover, I need to know the
plane (G17/18/19) so that I know which of the three coordinates is the angle
coordinate.

While I'm at it, this layer also checks for syntax errors involving choice
of plane (G17/18/19) and circular interpolation. For instance, normally
(with G17), you shouldn't have a K value in the specification of the center
of a circular arc.

What's left for higher layers:
* Eliminating TLO and work offsets. If I can do this sooner, it would be nice.
