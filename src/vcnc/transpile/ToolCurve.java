...
Base class for the two types of tool curve: linear moves and arc moves.
These objects are not the same as Statement objects. They are used when
there's a need for hairier calculation, like when dealing with cutter
comp or producing the signals to move the axes.

Aside from simplifying the code in Layer05, this class is also meant as
the place where the more mathematical/geometric aspects of the problem
are handled.

This code has some history. There was a much earlier Java version where I
tried out different ideas, then a Java version where many of these ideas
were taken out, then I moved to C++ and cleaned it up, then moved back to
this Java version.

I am not happy with the way the class organization turned out. I don't 
see any way to avoid having this class "know about" its sub-classes,
ArcCurve and LineCurve.
