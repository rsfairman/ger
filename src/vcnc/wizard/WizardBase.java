...
Regarding many of the issues mentioned below, wait to consider them until
I have every done *except* cutter comp. I'm afraid that a series of weird
circumstances that I haven't considered will keep cropping up so that
any fixes that appear here will lead to a lot of wheel spinning. I *think*
that, if wizards are expanded as they appear in the code, then there will
be no issues with letting them generate any G/M code at all.
...
Fully transpile code up to a wizard invocation -- *fully*, including cutter 
comp, to the final form. This allows *all* aspects of state to be accessible
to the wizard. Expand the wizard. Transpile it *fully*. Continue to transpile
post-wizard code until the next wizard, etc.

This way, the wizard is still able to use all available G-codes, even though
it is (in some sense) expanded at the end. 

An additional thing wizards should have access to is are saveState() and
restoreState() methods. These might come in various versions since it may be
that not all aspects of state should be saved/restored, like the (X,Y,Z) 
position. 
...
abstract public class WizardBase {
  ...
  private static ArrayList<Statement> wizout = null;

  // Define the wizard here.
  abstract public void definition(ArrayList<Object> args);
  
  
  public ArrayList<Statement> execute(ArrayList<Object> args) {
  ...
  public static void G00() {
    ...
    wizout.add(new Statement(Statement.G00));
  ...
  public static void G01() {
    ...
    wizout.add(new Statement(Statement.G01));
